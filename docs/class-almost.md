...
注解是一种特殊的接口，没有直接作用，只是为特定部分的代码提供额外的信息，供开发者、编译器等使用。

可以自定义接口，步骤是：先指明注解能出现在哪里，然后是保留原则，最后是注解的名称。

常用的基本注解包括：@Override, @Deprecated, @SuppressWarning

### 泛型

泛型语法指定某个容器中的实例的类型，方便编译器捕获不安全的代码。

### 嵌套类

嵌套类，又称内部类，包括：静态成员，非静态成员，局部类，匿名类。

本质上，嵌套类与普通类编译后并没有区别，只是使用了特殊的访问权限。

### 流式表达

流式表达用于集合。Stream对象从集合对象上生成，使用管道的方式执行一系列操作，最终会生成一个结果，或使用 Collector 对象再次具化为真正的集合。

### lambda表达式

lambda表达式引入自函数式语言，句法是 `(p, q) -> {}`。
Java中的lambda表达式本质上还是基于类，将只有一个强制方法的接口转为lambda表达式。

常用的 lambda 模式包括： map, filter, reduce 。
- map() 方法使用接口是 `Function<T,R>`，也是在 java.util.function 中定义，只有一个默认方法 apply()。
- filter() 方法使用的接口是 `Predicate<T>`, 执行一个 test 操作。
...
java.util 中定义了 Observable 类。（注意，不是接口！）
从其方法名称来看，正是实现了“观察者”设计模式。暂时不确定是否还有其他的设计模式被实现。
